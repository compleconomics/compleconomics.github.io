...
for u in K.nodes(data=True):
    for v in K.nodes(data=True):
        if u[0] != v[0]:
            if (v[0] in K[u[0]]):
                G.add_edge(u[0],v[0],value=1,sourcetype=u[1]["club"],targettype=u[1]["club"], time="Club Membership")
        ...
        if u[0] != v[0]:
            if (v[0] in K[u[0]]):
                for c in range(len(comm)):
                    ...
                    if (v[0] in comm[c]):
                        ttype = c
                G.add_edge(u[0],v[0],value=1,sourcetype=stype,targettype=ttype, time="Community Detection")
